...
describe("RM-25 Repository Center", () => {
  ...
  it("projects normalized GitHub and Git lineage into the authenticated dashboard", async () => {
    ...
    harness.setRepositoryCenterView(saved.id, buildRepositoryCenterView({ project: saved, github: result("github", github), git: result("git", git), now: "2026-09-02T09:00:00.000Z" }));
  ...
  it("renders degraded observations and never reports an offline worker as clean", async () => {
    ...
    const view = buildRepositoryCenterView({ project: saved, github: result("github", github), git: failed, gitSourceReference: "C:/controlled/subscription-agent", now: "2026-09-02T09:00:00.000Z" });
