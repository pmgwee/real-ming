...
| **Superseded** | Implemented and retained, but deliberately not activated because another live path meets the guarantee |
...
| Curated-knowledge guarantees | **Live** | Stated as a guarantee, not an implementation. Versioned atomic publication, candidate quarantine, access-controlled cross-domain projection, Trust-Domain scoping and restore-safe forgetting are all delivered by the role-scoped path above and exercised in production. The original six-root encrypted vault and Projection Broker of [ADR-0018](docs/adr/0018-compile-knowledge-into-trust-domain-vaults.md) are **superseded as an activation target** by [ADR-0024](docs/adr/0024-supersede-the-six-root-curated-vault-with-the-role-scoped-path.md): retained in the repository, never activated, and deliberately so. Two guarantees are **accepted as not required** and are therefore not provided: encryption at rest across six separate roots, and a distinct Projection Broker component. |
...
| [docs/adr/](docs/adr) | Twenty-four decision records, oldest to newest |
| [docs/architecture/real-ming-agent-diagram-v8-production.html](docs/architecture/real-ming-agent-diagram-v8-production.html) | The current architecture diagram — Revision 6, v8 production |
