...
- The post-RM-40 selective consolidation design uses a native Hermes cron job and native LLM-Wiki/Obsidian skills. It processes only explicit or deliberately marked candidates; it does not intercept ordinary turns, scan every session, or reinterpret native memory deletion. Each candidate has exactly one explicit `trustDomain`; synthesis may not infer, omit, or combine domains.
...
- The optional advanced Projection Broker releases Compiled Knowledge only for the current Executive Role, Work Item, purpose, and allowed Trust Domains. CTO and CMO receive distinct views over Ming Creatives; Entertainment remains isolated even without a dedicated Executive Role. Native-first generated wiki retrieval requires one Executive Role plus one canonical Trust Domain and validates the pair before resolving or reading generated pages: COO/Personal, CTO/Ming Creatives, CMO/Ming Creatives, CAO/Academic, and Personal CFO/Finance. CEO is not an Executive Role or capture domain; CEO cross-domain material remains an Approved Projection. Entertainment has no role-scoped reader.
...
- The Knowledge Vault contracts in this section — Candidate Envelope provenance, contradiction quarantine, versioned publication, `index.md`, append-only `log.md`, linting and Trust-Domain projection — are retained as optional stronger guarantees. Default persistent knowledge uses the native Obsidian and LLM Wiki workflow over an absolute vault path plus the reviewed selective-consolidation design's bounded registry and role-scoped `wiki_retrieve` gate; Tasks 0–8 of that follow-on design are controlled-tested, but there is no production caller or active cron row. Role playbooks may request their fixed domain only when durable knowledge is relevant and the tool is available; they do not retrieve on every turn, and unavailable generated knowledge does not disable ordinary Hermes reasoning. A retained guarantee must be wired to a real production caller and proven end to end before it is claimed.
