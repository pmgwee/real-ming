...
2026; live operations and role-scoped native knowledge activated 14 September
2026).** The Malaysia West host `real-ming-control-plane-my` runs image
`real-ming:v6-c4ee6eb` with
...
7 September. Nine MCP servers are enabled, of which the Real-Ming extension
contributes 17 — verified by `hermes mcp test real-ming` on 14 September —
covering Work Items including governed capture, scheduled reports, calendar
read and create, mail search, read and draft across three Google accounts, and
the seven native-knowledge operations.
...
passed; the 14 September backup additionally carries the live native-knowledge
