...
| **Dashboard access from your phone** | The dashboard accepts only a loopback `Host` and 400s everything else, so a phone needs Tailscale plus an SSH client doing a local port forward. Making it a normal URL instead means a non-loopback bind with a real auth provider — which changes `control-plane-deployment-preflight.ts`, contradicts `docs/BASELINE.md`, and needs an ADR. | Stay on the tunnel for now. Telegram is already the phone interface that matters; the dashboard is for inspection. Decide whether it earns its keep at all (milestone 9) before paying for an ADR. Full reasoning and the measured evidence are in [Part 3 of the access runbook](dashboard-and-obsidian-access-runbook.md). | ⏳ Open — decision 3 |
