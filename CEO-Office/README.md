...
**Milestone 5 — done, 7 September. Native cron owns your daily reports.** Hermes now calls Real-Ming as a tool and gets your task vocabulary back. Asked how many Work Items exist and how many wait on you, it answered **33 total, 3 waiting on your decision** — matching the database exactly, and correctly treating "Ready for CEO Review" as *not finished*. That semantic is the whole reason Real-Ming still exists. Ownership switched live on 7 September; the duplicate guard was proven when the native run correctly sent nothing because the brief had already gone out. [Milestone 5 evidence](../docs/evidence/RM-40-v6-milestone-5-extension-evidence.md) and [scheduler evidence](../docs/evidence/RM-40-v6-milestone-5-scheduler-ownership-evidence.md).
...
**Milestone 7 — done, 7 September, bar your usefulness review.** Hermes reachability, Telegram connection, 59 enabled skills, the `real-ming` MCP server and zero native cron jobs are visible without copying native sessions or inventing legacy conversation state. The two dashboards are complementary, not duplicative, and nothing crosses that should not: Real-Ming carries five metadata fields about Hermes and no secret, reasoning or session content. This also settles `src/dashboard/` as **keep**. [Dashboard comparison](../docs/evidence/RM-40-v6-milestone-7-comparison-2026-09-07.md).

**Milestone 8 — Notion read-back and honest failure verified live, 7 September; the DuitSini coding tracer is blocked on a repo checkout and git credential.** **Milestone 9 — restart/reconcile live-verified: nothing lost, nothing replayed; usage, cost and retention reviewed.** See [milestone 8 evidence](../docs/evidence/RM-40-v6-milestone-8-acceptance-evidence.md), [milestone 9 evidence](../docs/evidence/RM-40-v6-milestone-9-recovery-evidence.md) and the [deployment evidence](../docs/evidence/RM-40-v6-deployment-evidence-2026-09-07.md).
...
| Next engineering work | None blocking. Closeout waits on the first unattended cron fire, the DuitSini credential decision and your reviews. |
| Waiting on you | Tonight's 21:30 roll-up landing on its own; the DuitSini checkout/credential decision; your dashboard usefulness review; opening the vault in Obsidian; and a yes on the RM-40 issue criteria. See [Phase 4 CEO actions](phase-4-ceo-action.md). Leave the East Asia VM stopped but undeleted; nothing to do in Tailscale. |
...
| **Create Notion tasks from Telegram** | The agent can read Work Items and link execution to them, but cannot create one. Adding it means a `real_ming_create_work_item` tool plus a Notion write credential on the Hermes host — the first credential there beyond the bridge key. | Approve if you want "remember to do X" in Telegram to become a real Master Task. It is small and well-bounded, but it is a behaviour change and a new credential, so it is yours to choose. | ⏳ Open — see [the impact audit](../docs/evidence/RM-40-hermes-operation-impact-audit-2026-09-07.md) |
