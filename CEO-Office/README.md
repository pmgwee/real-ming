...
| | |
| --- | --- |
| Phase 3 tickets closed | **44 of 44** — RM-40 (#41) closed 8 Sep 2026 |
| Startable by an agent right now | **None.** `npm run graph:status` selects no next node |
...
| **Optional curated-knowledge guarantees** | Versioned atomic publication, contradiction quarantine and access-controlled cross-domain projection are built and controlled-tested but were never wired to a production caller. Revision 6 makes them optional rather than default. | Milestone 6 is done and the role-scoped native path is live, so the deferral can now be settled. **Most of the guarantees in this row are already met live** by that path — versioned atomic publication, quarantine, Trust-Domain-gated projection and restore-safe forgetting. Only encryption-at-rest across six roots and a separate Projection Broker are not. **Recommendation: retire the row as superseded** rather than run a second knowledge system. | ⏳ **Open — decision 2, and the only thing stopping every capability reading `Live`.** Not a credential problem: `real-ming-vault-key` already exists in Key Vault and resolves. The single gap is that `control-plane-cli.ts` never passes `knowledgeOperations` — see the [curated vault runbook](curated-knowledge-vault-activation-runbook.md) |
...
| [curated-knowledge-vault-activation-runbook.md](curated-knowledge-vault-activation-runbook.md) | **One decision.** The only capability row that does not read `Live`, and why it is not a credential problem — the vault key already exists. Retire the row as superseded (recommended) or activate the vault |
