...
| [curated-knowledge-vault-activation-runbook.md](curated-knowledge-vault-activation-runbook.md) | **Blocked on you.** The one capability row that cannot read `Live`, why, and the two honest options — retire it as superseded (recommended, no credential) or provision `REAL_MING_VAULT_KEY` and activate it |
