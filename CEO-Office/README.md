...
| **Install the native Notion connector** | You were right that Hermes has one. `hermes mcp install notion` connects the official Nous-approved catalog entry to `mcp.notion.com` over **OAuth 2.1** — no token stored on the host, no Real-Ming code, full read/write on pages and databases. It is the same shape as Claude Code's Notion connector. | Approve and I will run the install; the browser authorization step is yours (Hermes prints a URL, you approve and paste the redirect back). This replaces my earlier suggestion of building a Real-Ming tool, which was the wrong shape. | ⏳ Open — one browser authorization from you; see [the impact audit](../docs/evidence/RM-40-hermes-operation-impact-audit-2026-09-07.md) |
