...
3. If CEO-Office documents an unmet human action, stop and present that action plainly. Resume only after it is satisfied.
...
6. Obtain independent Standards and Spec reviews against the previous commit. Reproduce each suspected correctness defect with a failing approved-seam test before fixing it.
7. Commit only that ticket, push, close its issue with evidence, recompute the graph, and immediately continue.

SUBAGENTS
Use subagents when they materially increase confidence: independent Standards and Spec reviews, bounded read-only code/ADR/provider investigations, or isolated test analysis. Keep one implementation owner for the ticket. Never let parallel agents edit shared files or independently mutate live systems. Give each subagent a narrow deliverable and fixed comparison point; the primary agent integrates findings and owns the final diff.
...
Work on feat/tracer-1-daily-operations and open one PR per milestone, never one growing branch. Announce every pull request in your reply with its number, contents, and your recommendation, and add it to the Decisions table in CEO-Office/README.md. Never let a PR appear that I have to discover myself.
...
Ask before any outward-facing or hard-to-reverse live action. If CEO-Office shows an unmet human action, or the selected ticket reaches a CEO-only decision, stop immediately: state the exact action, create/update the CEO-Office runbook and status board, preserve partial work, and wait. Preparation or reconciliation review is not approval to mutate.
